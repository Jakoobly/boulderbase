  ...
  return <main className="screen active"><div className="topbar"><button className="back-btn" onClick={back}>← zurück</button><div className="logo" style={{ fontSize: 18 }}>Profil</div></div>
    ...
    {deleteConfirmOpen && <div className="delete-popup-backdrop" onClick={() => setDeleteConfirmOpen(false)}>
      <div className="delete-popup" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="compact-delete-actions"><button type="button" className="tiny-btn" onClick={() => setDeleteConfirmOpen(false)}>Abbrechen</button><button type="button" className="tiny-btn danger-tiny-btn" onClick={async () => { setDeleteConfirmOpen(false); setPersonalizeOpen(false); await deleteProfile(); }}>Endgültig löschen</button></div>
      </div>
    </div>}
  </main>;
}